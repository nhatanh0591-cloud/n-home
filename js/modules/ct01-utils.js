...
/**
 * @param {object} building - cần building.address, landlordSignatureImage, landlordName, landlordIdNumber
 * @param {object} customer - cần customer.name, birthDate, gender, idNumber
 * @param {string} [customerSignatureDataUrl] - chữ ký khách
...
export function buildCT01Html(building, customer, customerSignatureDataUrl, residenceUntilDate, signDate, includePhone) {
    ...
    const landlordSigHtml = building?.landlordSignatureImage
        ? `<img src="${building.landlordSignatureImage}" style="height:98pt;max-width:130%;object-fit:contain;display:block;margin:0 auto;">`
        : '';
    const landlordFootHtml = `<p style="text-align:left;font-size:8.5pt;margin:0;white-space:nowrap;">(8) Họ và tên: ${building?.landlordName || dots(15)}</p><p style="text-align:left;font-size:8.5pt;margin:0;white-space:nowrap;">(8) Số định danh cá nhân:${building?.landlordIdNumber || dots(12)}</p>`;
...
<td style="width:${widthPct}%;text-align:center;vertical-align:top;padding:0 4pt 0 ${noLeftPad ? '0' : '4pt'};">
  <p style="text-align:left;margin:0;${italicDate ? 'font-style:italic;' : 'font-size:9.5pt;white-space:nowrap;'}">${dateLine}</p>
...
${sigCol(landlordSigHtml ? `..,ngày ${signDay} tháng ${signMonth} năm ${signYear}` : '..,ngày...tháng...năm...', 'Ý KIẾN CỦA CHỦ SỞ HỮU CHỖ Ở HỢP PHÁP<sup>(5)</sup>', landlordSigHtml,
    landlordFootHtml, 24, !landlordSigHtml)}
